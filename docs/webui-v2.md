...
- Let an admin act on anything needing a decision (action versions, access requests) in ≤ 2 clicks.
...
- Agent-facing surfaces — directory browsing, access requests, and action CRUD are REST API concerns; the console is admin-only.
...
| API equivalence | Console actions that admins can also perform by API (approvals, grants) accept the admin bearer token |

## 3. Console structure

A single dashboard: a **stats bar** on top (agent count, action counts by state, pending access requests, request totals by outcome) and **six tabs**. Tab switches and region updates are server-rendered fragments fetched via Datastar — no full page reloads.

| Tab | Purpose |
|---|---|
| **Dashboard/Directory** | All actions with all versions, lifecycle states, owning providers; entry point to per-action detail (§4) |
| **Access requests** | Cross-action queue of pending client access requests; approve/reject |
...
| **Audit trail** | Every state change: approvals, rejections, grants/revocations, agent registration/edits, key issuance/revocation |
| **Login** | (Not a tab — standalone page) |

## 4. Directory tab & per-action detail

### 4.1 Directory list

- All actions (active and deactivated), with owner, current active version, state of any pending version, per-action grant count, and recent invocation count.
...
### 4.2 Per-action detail (expanded inline or drill-in from a row)
...
2. **Access management** (grants for this action)
   - Clients with approved access (who/when granted).
   - Pending access requests for this action, with inline **Approve / Reject**.
   - **Revoke** on any existing grant (immediate effect).
   - **Admin-initiated grant**: proactively grant access to a client that has not requested it (bootstrap for trusted clients).
3. **Invocation log** (requests pre-filtered to this action)
   - Recent invocations with outcomes (success / request-rejected / provider-failure), expandable to full payloads, validation/arbitration results, and retry counts (same detail rendering as the Request log tab, §7).

**Consistency rule**: this detail view and the Access-requests tab read the same underlying grant/access-request store — they are two views (per-action and cross-action queue), never separate state.

## 5. Access requests tab

- Queue of **pending** access requests across all actions: client, action, provider, requested-at time, with Approve/Reject (optional notes).
- Also lists **recently decided** requests (approved/rejected with deciding admin and time) for context.
- Each row links to the action's detail view (§4.2) and the client's agent record (§6).
...
- No manual actions here — the log is observational; the only human gates are upstream approvals (Directory / Access requests tabs).
...
- Chronological record of every **state change** in the system:
  ...
  - Access: request submitted / approved / rejected; grant revoked; admin-initiated grant
...
| GET | `/admin/partials/access-requests` | Access request queue fragment | session |
| POST | `/admin/access-request/<request_id>/approve` | Approve an access request | session / admin token |
| POST | `/admin/access-request/<request_id>/reject` | Reject an access request | session / admin token |
| POST | `/admin/action/<action_id>/grant` | Admin-initiated grant (client + action) | session / admin token |
| POST | `/admin/grant/<grant_id>/revoke` | Revoke an existing grant | session / admin token |
...
- **Data**: all content comes from the shared storage layer (agents, actions/versions, grants/access requests, request log, audit events) via the same queries the API uses — the console never keeps its own copy of state.
...
- **Logging**: all console actions are logged and also recorded as audit events (§8).
...
- Any pending action version or access request can be resolved in ≤ 2 clicks, with optional notes.
...
## 13. Beyond the scope of this document

- **Agent-facing API design** — directory, access-request, and action CRUD endpoints for clients/providers.
