...
**Agent DMZ** (`admz`) is a demilitarized zone (DMZ) between an **untrusted external agent** and a **trusted internal agent**. The two sides never talk directly. All communication passes through a schema-validated, LLM-arbitrated gateway with persistent queues and a human review path for anything suspicious.
