# LLM DMZ v2 — Implementation Todo List

> **Read this first:** To have sufficient context, you must read the v2 PRD set indexed in [`docs/index-v2.md`](docs/index-v2.md) — `system-prd-v2.md`, `rest-api-v2.md`, `dispatch-v2.md`, `schemas-v2.md`, `webui-v2.md`, and `infra-v2.md` — before starting any task below. Each PRD is standalone; task references to `#N` point to numbered answers in `docs/v2-implementation-clarifications.md`, which is also required reading.
...
- [ ] T1.1 Implement `agents` + `audit_events` SQLAlchemy models per ERD (#13)
- [ ] T1.2 Implement `actions` + `action_versions` + `enrollments` models per ERD (delivery configs inside `action_versions.payload`)
- [ ] T1.3 Implement `requests` + `dispatch_attempts` models per ERD with all indexes
- [ ] T1.4 Create single baseline Alembic migration from the models (#32)
- [ ] T1.5 **Validate:** migration upgrade/downgrade round-trips clean on SQLite; schema spot-checked against #13; optional Postgres pass for portability
- [ ] T1.6 Build storage module: CRUD + list/get with pagination for agents, actions, versions, enrollments
- [ ] T1.7 Build storage module: request/dispatch-attempt logging writes + stats queries (all-time by outcome, trailing 24h, #26)
- [ ] T1.8 **Validate:** storage unit tests (pagination bounds, unique constraints, stats math with fixed timestamps)
- [ ] T1.9 Build audit helper (append-only `audit_events` writer with actor/event/target/detail)
- [ ] T1.10 Implement key generation: `dmz_`/`dmzadm_` prefixes, `token_urlsafe(32)`, SHA-256 hex storage (#15)
- [ ] T1.11 Implement bearer auth middleware resolving any Bearer token to agent-or-admin identity (#17)
- [ ] T1.12 **Validate:** unit tests — reveal-once semantics, hash lookup, unknown/garbage token → 401, prefix namespaces don't collide

## Phase 2 — Schema Registry & Action Lifecycle
...
- [ ] T2.8 **Validate:** API tests — full lifecycle (pending accepts PUT after rejection #7, withdrawn reversible, supersede flow, ownership 403s, hidden-action 404s)
...
- [ ] T2.12 **Validate:** API tests for filters/pagination/projections per #18–20
...
## Phase 3 — Arbitration & Dispatch

- [ ] T3.1 Define `ArbiterClient` + `ProviderTransport` interfaces (the DI seam for offline tests, #31)
- [ ] T3.2 Implement verdict parser (JSON → first-`{...}` fallback, bool coercion, default reason; unparseable = failed check)
...
- [ ] T3.10 **Validate:** transport unit tests with fake HTTP/subprocess — framing exact-match, timeout, per-provider retry/timeout override of defaults (#28)
- [ ] T3.11 Implement invoke pipeline happy path: validate → request arbiter → dispatch → response validate → response arbiter → 200 result
...
- [ ] T3.13 Implement retry loop: response-side validation/arbitration per attempt, error injection, exhaustion → `502 provider_failed`; request-side check runs exactly once (#4)
- [ ] T3.14 Implement `requests` + `dispatch_attempts` logging (framing, timestamps, `error_class`/`detail` per attempt, #13)
- [ ] T3.15 **Validate:** integration tests with injected fakes — every outcome path, retry-injection content, retry exhaustion, arbiter outage on both sides, immutable request verdict across retries
...
- [ ] T4.3 Implement auth guard decorator (bearer first, else session; agent key → 403/redirect per #17)
...
- [ ] T4.14 Build pending-version approval queue: approve/reject + optional notes (`decision_notes`), activation incl. withdrawn→active path
- [ ] T4.15 Build enrollment queue: approve/reject + notes, revoke, admin-initiated enroll, `POST /admin/enrollment/<id>/reset` (#11)
- [ ] T4.16 **Validate:** queue tests — ≤2-click flows, notes persisted, multi-patch updates queue+stats together, audit rows with actor
...
## Phase 5 — Hardening & Cutover Prep

- [ ] T5.1 E2E scenario 1: provider registers → admin approves → client enrolls → admin grants → invoke succeeds (all fakes)
- [ ] T5.2 E2E scenario 2: withdrawal → 404 on invoke → new version approved → auto-reactivation, enrollment still valid (#8)
- [ ] T5.3 E2E scenario 3: version supersede, enrollment rejection → admin reset → re-request (#11), retry exhaustion, arbiter outage paths (#1)
- [ ] T5.4 **Validate:** full suite + lint + typecheck green in CI; `fab test` from a clean checkout
- [ ] T5.5 Write `deploy/README.md` checklist (gunicorn `-w 2 --threads 16 --timeout 900`, `proxy_read_timeout` ≥ 900s, OpenRouter key checks, `internal_error` alerting, SQLite sizing/growth note, vendor upgrade step)
- [ ] T5.6 Final compose validation: deterministic config check, entrypoint order (migrate → serve), documented smoke-run steps

---

## Dependencies & assignment notes

- T0.* block everything; T1 blocks T2–T4; T2.1 → T2.2 → T2.4+; T3.1 before T3.6–T3.9; T3.11–T3.13 need T3.6–T3.9 and T1.7; T4.* need T1/T2 storage and lifecycle; T5.* last.
- **Owner-review gates:** T2.17 (skills) and T3.5 (arbiter prompts) — do not merge without project-owner sign-off.
- Suggested split for 4 juniors: J1 = Phase 0 (+infra ownership throughout); J2 = Phase 1 + T2.1–T2.8; J3 = T2.9–T2.17 + Phase 4; J4 = Phase 3 (strongest junior; pair with owner on prompts).
