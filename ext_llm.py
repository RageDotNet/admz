"""External (untrusted) LLM agent with mock email access via LiteLLM + OpenRouter."""
...
from emailtool import EMAIL_TOOLS, run_tool
from llm_logging import get_logger, log_json

load_dotenv()

logger = get_logger("ext_llm")

MODEL = "openrouter/openai/gpt-oss-120b:free"

SYSTEM_PROMPT = """You are the external untrusted agent handling outside communication.
You can read incoming email from the inbox and send replies or new messages.
Use the email tools to check for new mail, review messages, and send responses.
Summarize clearly and only take email actions that match the user's request."""
...
def main() -> None:
    ...
    print("External LLM (email). Type 'quit' to exit.")
    while True:
        try:
            user_input = input("\nYou: ").strip()
